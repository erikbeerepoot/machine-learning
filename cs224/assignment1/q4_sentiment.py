...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    sentVector = np.sum(sentence_vectors, axis=0,keepdims=True).reshape((wordVectors.shape[1],))
...
def chooseBestModel(results):
    ...
    scores = [result["test"] for result in results]
    min = np.min(scores)
    min_index = scores.index(min)
    print(results[min_index])
    bestResult = results[min_index]
